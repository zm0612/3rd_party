...
class S2CellId {
 public:
  ...
  explicit IFNDEF_SWIG(constexpr) S2CellId(S2::uint64 id) : id_(id) {}
  ...
  static constexpr S2CellId Sentinel() { return S2CellId(~S2::uint64{0}); }
  ...
  static S2CellId FromFacePosLevel(int face, S2::uint64 pos, int level);
  ...
  S2CellId advance_wrap(S2::int64 steps) const;
  ...
  S2::uint64 lsb() const { return id_ & (~id_ + 1); }

  // Return the lowest-numbered bit that is on for cells at the given level.
  static S2::uint64 lsb_for_level(int level) {
    return S2::uint64{1} << (2 * (kMaxLevel - level));
  }
 ...
 private:
  ...
  static const S2::uint64 kWrapOffset = static_cast<S2::uint64>(kNumFaces) << kPosBits;
...
inline S2CellId S2CellId::FromFace(int face) {
  return S2CellId((static_cast<S2::uint64>(face) << kPosBits) + lsb_for_level(0));
}

inline S2CellId S2CellId::FromFacePosLevel(int face, S2::uint64 pos, int level) {
  S2CellId cell((static_cast<S2::uint64>(face) << kPosBits) + (pos | 1));
...
inline S2::uint64 S2CellId::pos() const {
  return id_ & (~S2::uint64{0} >> kFaceBits);
}
...
inline S2CellId S2CellId::parent(int level) const {
  ...
  S2_DCHECK_LE(level, this->level());
  S2::uint64 new_lsb = lsb_for_level(level);
